...
EOD;
	...
	function getDetailImg()
	{
		...
		if ($thumb_id != 0) { // existe imagen destacada
			...
			foreach ($thumbnail_details as $key => $value) {
				return [
					'status'	=>	true,
					'msg'		=>	'Procesado correctamente',
					'detail'	=>	[
						'url'			=>	esc_url($thumb_url[0]),
						'post_date'		=>	$value->post_date,
						'post_content'	=>	esc_attr($value->post_content),
						'post_excerpt'	=>	empty($value->post_excerpt) ?: ' - ' . esc_html($value->post_excerpt)
					]
				];				
			}
		}else{
