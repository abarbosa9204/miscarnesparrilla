...
class CostBudgetController extends Controller
{
    ...
    */
    function list_cost_budget()
    {
        ...
        *
        FROM vw_wpl_cost_budget where filter_field like '%" . $searchValue . "%' order by " . ((($columnName == 'cb_row_id') ? 'cb_row_id' : $columnName) . ' ' . $columnSortOrder) . " limit " . $rows . ',' . $rowperpage, OBJECT);

        $resulset2 = $wpdb->get_results("SELECT 
                                        *
        FROM vw_wpl_cost_budget where filter_field like '%" . $searchValue . "%'order by " . ((($columnName == 'cb_row_id') ? 'cb_row_id' : $columnName) . ' ' . $columnSortOrder), OBJECT);
        ...
        foreach ($resulset as $row) {
            ...
            if (!in_array($row->mime_extension, ['.html', '.php'])) {
                ...
                <a class="me-2" href="' . $row->cb_url . '" title="Descargar" download=""><i class="ti-cloud-down" style="color:#063970;font-size:22px;cursor:pointer"></i></a>
                <a class="ms-2" href="javascript:void(0);" onclick="showModalEditFile(' . "'" . $this->encryption($row->cb_row_id) . "'" . ')" title="Editar"><i class="ti-pencil-alt" style="color:#B61020;font-size:22px;cursor:pointer"></i></a>
                </div>';
            }
            
            $data[] = array(
                'cb_row_id' => $row->cb_row_id,
                'folder_name' => $row->folder_name,
                'div_path_id' => '<div id="' . $row->cb_row_id . '_' . $row->folder_name_in_server . '"></div>',
                'path_id' => $row->cb_row_id . '_' . $row->folder_name_in_server,
                'path_json' =>  $jstree,
                'cb_description' => $row->cb_description,
                'cb_text_information' => $row->cb_text_information,
                'cb_status' => ($row->cb_status == 1 ? '<span class="badge bg-success twentyseventeen-font-size-theme-15-5">Activo</span>' : '<span class="badge bg-danger twentyseventeen-font-size-theme-15-5">Inactivo</span>'),
    ...
    * @return JSON
    */
    function uploadFilesCostBudget()
    {
        global $wpdb;
        ...
        if (in_array($type, $arr_img_ext)) {
            ...
            $row_id = $wpdb->get_row("SELECT max(cb_row_id) as cb_row_id FROM wpl_cost_budget", OBJECT);
            ...
            if ($row_type_id) {
                ...
                if (file_exists($filePath)) {
                    $insert = $wpdb->insert('wpl_cost_budget', [
                        'cb_row_id'                     =>  $row_id->cb_row_id + 1,
                        ...
                        'cb_description'                =>  $name_file_upload,
                        'cb_text_information'           =>  $text_file_description,
                        'cb_name_file'                  =>  $path_save  . $fileName,
                        'cb_url'                        =>  get_template_directory_uri() . $file_path_save . '/' . $path_save . $fileName,
                        'cb_status'                     =>  1,
    ...
    */
    ...
    {
        global $wpdb;
        $dataFile = $wpdb->get_row("SELECT * FROM vw_wpl_cost_budget WHERE cb_row_id ='" . $this->decryption($this->param) . "'", OBJECT);
        if (!$dataFile) {
            return [
                'status'    =>  'error',
        ...
        $response = [
            ...
            'msg'       =>  'Procesado correctamente',
            'data'      =>  [],
            'id'        =>  $this->encryption($dataFile->cb_row_id),
            'cb_description'    =>  $dataFile->cb_description,
            'cb_text_information'    =>  $dataFile->cb_text_information,
            'cb_status'    =>  $dataFile->cb_status
    ...
    * @return JSON
    */
    function uploadFilesCostBudgetEdit()
    {
        global $wpdb;
        $data = $this->array_json_stringify($this->request['params']);
        $folder_row_id = $subfolder_n1_row_id = $subfolder_n2_row_id = $subfolder_n3_row_id = $subfolder_n4_row_id = $subfolder_n5_row_id = $name_file_upload = $text_file_description = null;
        ...
        $dataFile = $wpdb->get_row("SELECT * FROM vw_wpl_cost_budget WHERE cb_row_id='" . $id_file_edit . "'", OBJECT);
        ...
        if ($this->files['file'] !== null) {
            ...
            if (in_array($type, $arr_img_ext)) {
                $old_url = $path_origin . $dataFile->folder_name_in_server . '/' . $dataFile->cb_name_file;
                $row_type_id = $wpdb->get_row("SELECT mime_row_id FROM wpl_mime_type WHERE mime_type ='" . $type . "'", OBJECT);
                if (!$row_type_id) {
                    return [
                        'status'    =>    'warning',
                    ...
                    if (file_exists($filePath)) {
                        $update = $wpdb->update(
                            ...
                            [
                                ...
                                'cb_name_file'                  =>  $path_save  . $fileName,
                                'cb_url'                        =>  get_template_directory_uri() . $file_path_save . '/' . $path_save . $fileName,
                                'cb_status'                     => ($file_upload_status != 1 ? false : true),
                                'updated_at'                    =>  date('Y-m-d H:i:s'),
                                'user_login_id_update'          =>  get_current_user_id()
                            ],
                            ['cb_row_id' => $dataFile->cb_row_id]
                        );
                        if ($update) {
                            return [
                                'status'    =>  'success',
        ...
        if ($update) {
            if ($old_arrar != $new_array) {
                $file_path_exists = $path_origin . $dataFile->folder_name_in_server . '/' . $dataFile->cb_name_file;
                if (file_exists($file_path_exists)) {
                    //Data actualizada
                    $dataRename = $wpdb->get_row("SELECT * FROM vw_wpl_cost_budget WHERE cb_row_id='" . $dataFile->cb_row_id . "'", OBJECT);
                    ...
                    //Nuevo nombre
                    $new_name = str_replace($old_path, $new_path, $dataFile->cb_name_file);
                    ...
                    if (rename($old_url, $new_url)) {
                        $updateUrlFiles = $wpdb->update(
                            'wpl_cost_budget',
                            [
                                'cb_name_file'  =>  $new_name,
                                'cb_url'        =>  get_template_directory_uri() . '/template-parts/uploads/files/' . $dataFile->folder_name_in_server . '/' . $new_name,
                            ],
                            ['cb_row_id' => $dataFile->cb_row_id]
                        );
                        if ($updateUrlFiles) {
